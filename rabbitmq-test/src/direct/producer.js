...
async function main() {
  ...
  // 声明交换机 （durable: true 这个交换机在 RabbitMQ 重启后还活着）
  ...
  for (const task of tasks) {
    ...
    channel.publish(
      EXCHANGE, // 交换机名
      task.routingKey, // 地址
      Buffer.from(JSON.stringify(task.body)), // 消息内容
      { persistent: true, contentType: 'application/json' }, //属性（persistent为true，写进磁盘，即使挂了也不能丢，contentType 标明格式）
    );
    console.log(
