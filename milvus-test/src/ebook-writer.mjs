...
// 从文件名提取书名（去掉扩展名）
const EBOOK_NAME = parse(EPUB_FILE).name
...
async function getEmbedding(text){
    const result = await embedings.embedQuery(text)
    return result
}
    ...
    try {
        ...
        const insertData = await Promise.all(
            chunks.map(async (chunk, chunkIndex) => {
                ...
                // 手动生成 ID: book_id_chapterNum_indx
                return {
...
async function loadAndProcessEPubStreaming(bookId) {
    try {
        ...
        console.log(`✔️ 加载完成，共 ${documents.length} 个章节\n`)
        ...
        for (let chapterIndex = 0; chapterIndex < documents.length; chapterIndex++){
            ...
            console.log(`  ✔️ 已插入 ${insertedCount} 条记录（累计： ${totalInserted}）\n`)
        }
