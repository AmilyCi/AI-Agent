...
const HybridRetrievalState = Annotation.Root({
  query: Annotation(), // 用户问题
  queryAugmentation: Annotation(), // 根据 query 拆分的三个相似问题
  esHits: Annotation(), // es检索命中数据
  milvusHits: Annotation(), // 向量检索命中数据
  merged: Annotation(),
...
export function compileHybridRetrievalGraph(
  ...
  return new StateGraph(HybridRetrievalState)
    .addNode('query_augment', async (state) => ({
      // 这里的 query 是用户问题，需要先通过 LLM 重写为 3 条多角度问句
      queryAugmentation: await augmentQuery(chatModel, state.query ?? ''),
    }))
    .addNode('es_recall', async (state) => {
      const qs = retrievalQueryStrings(state.query, state.queryAugmentation); // 原始 + 拆分的问题
      const n = Math.max(1, qs.length);
      const kEach = Math.max(2, Math.ceil(ES_K / n)); // 每个问题最多召回 kEach 条
