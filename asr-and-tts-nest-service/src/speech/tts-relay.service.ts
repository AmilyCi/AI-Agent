...
export class TtsRelayService implements OnModuleDestroy {
  ...
  registerClient(clientWs: WebSocket, wantedSessionId?: string): string {
    ...
    // 防御性处理，保证不管 close 时间什么时候到，都不回有两个相同 sessionId 共存的情况
    if (existing) {
      this.closeSession(sessionId, 'client reconnected');
    }

    // 创建新的 session 并存入 Map
    this.sessions.set(sessionId, {
      sessionId,
      clientWs, // 客户端的 websocket
      ready: false, // 腾讯云还没就绪
      pendingChunks: [], // 等待发送的文本
      closed: false, // 未关闭
    });

    // 给客户端推送消息，告诉你的 sessionId 是多少
  ...
  // 腾讯云 TTS 的 WebSocket 连接建立，并发送初始化消息，如果已连接，不需要处理
  ...
  // 把文本发送给腾讯云，如果腾讯云还没准备好，先存到 pendingChunks 中
  ...
  // 腾讯云签名，生成可流式访问的 WebSocket 地址，用于文字转语音（TTS）
